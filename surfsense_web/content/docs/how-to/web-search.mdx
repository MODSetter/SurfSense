...
You can also add live search connectors such as Baidu Search, Tavily, Linkup, and Oxylabs to a search space. When those connectors are active, SurfSense queries them in parallel with SearXNG and merges the results before passing sources to the assistant.
...
| Oxylabs | General web search plus single-URL extract via Oxylabs AI Studio | [Oxylabs connector](/docs/connectors/oxylabs) |
