...
description: Building NeoNote from source using docker-compose.dev.yml
---

If you're contributing to NeoNote and want to build from source, use `docker-compose.dev.yml` instead:
...
| `PGADMIN_DEFAULT_EMAIL` | pgAdmin login email | `admin@neonote.com` |
| `PGADMIN_DEFAULT_PASSWORD` | pgAdmin login password | `neonote` |
