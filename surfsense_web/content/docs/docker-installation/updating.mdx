---
title: Updating
description: How to update your NeoNote Docker deployment
---
...
docker run --rm -v /var/run/docker.sock:/var/run/docker.sock \
  ...
  --label-filter "com.docker.compose.project=neonote"
```
...
```bash
cd neonote   # or NeoNote/docker if you cloned manually
...
<Callout type="warn">
If you were previously using `docker-compose.quickstart.yml` (the legacy all-in-one `neonote` container), your data lives in a `surfsense-data` volume and requires a **one-time migration** before switching to the current setup. PostgreSQL has been upgraded from version 14 to 17, so a simple volume swap will not work.
