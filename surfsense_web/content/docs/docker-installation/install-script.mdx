...
description: One-command installation of NeoNote using Docker
---
...
This creates a `./neonote/` directory with `docker-compose.yml` and `.env`, then runs `docker compose up -d`.

To skip Watchtower (e.g. in production where you manage updates yourself):

```bash
curl -fsSL https://raw.githubusercontent.com/MODSetter/NeoNote/main/docker/scripts/install.sh | bash -s -- --no-watchtower
```
...
After starting, access NeoNote at:
