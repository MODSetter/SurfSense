...
- Detects a legacy all-in-one installation and migrates its data automatically (see [Updating](/docs/legacy/docker-installation/updating#migrating-from-the-all-in-one-container)).
...
- **Connector credentials** — OAuth apps for [external connectors](/docs/legacy/connectors/external) that need them on self-hosted deployments.
- **Messaging channels** — Telegram, WhatsApp, Slack, and Discord bots (see [Messaging Channels](/docs/legacy/messaging-channels)).
...
To update SurfSense, see [Updating](/docs/legacy/docker-installation/updating).
...
There's also a `docker-compose.deps-only.yml` that runs just the dependencies (Postgres, Redis, zero-cache) in Docker while you run the backend and frontend natively — see [Manual Installation](/docs/legacy/manual-installation#alternative-let-docker-manage-all-dependencies).
