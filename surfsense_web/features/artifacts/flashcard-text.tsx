...
import { parseFlashcardText } from "./flashcards-schema";
...
export function FlashcardText({ content, className }: { content: string; className?: string }) {
	const segments = useMemo(() => {
		const parsed = parseFlashcardText(content);
		if (!parsed) throw new Error("Flashcard text violated the verified contract");
		return parsed;
	}, [content]);
