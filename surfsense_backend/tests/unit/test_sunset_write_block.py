...
@pytest.mark.parametrize(
    "path",
    ["/api/v1/license/resend", "/api/v1/license/trial", "/api/v1/stripe/webhook"],
)
...
def test_the_scraper_api_keeps_running(client, sunset_on):
    """It serves PATs through the gap week and becomes the paid plugin at T+7.

    The workspace id sits mid-path, so a prefix allowlist would miss it and
