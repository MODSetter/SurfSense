"""The video backfill only touches its own tool parts and carries the id."""

from __future__ import annotations

import pytest

from scripts.backfill_video_artifacts import _repoint, _tool_parts
...
def test_tool_parts_matches_only_video_tool_calls_with_a_result():
    content = [
        ...
        {"type": "tool-call", "toolName": "generate_video_presentation"},  # no result
        {
            ...
            "result": {"video_presentation_id": 7},
        },
    ]
    ...
    assert len(parts) == 1
    assert parts[0]["result"]["video_presentation_id"] == 7


def test_repoint_adds_ids_without_dropping_the_legacy_id():
    result = {"status": "ready", "video_presentation_id": 7, "title": "Demo"}
    ...
    assert repointed["video_presentation_id"] == 7
