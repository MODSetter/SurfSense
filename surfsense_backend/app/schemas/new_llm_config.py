...
class LLMPreferencesRead(BaseModel):
    ...
    video_llm_id: int | None = Field(
        None, description="ID of the LLM config to use for video code generation"
    )
    ...
    video_llm: dict[str, Any] | None = Field(
        None, description="Full config for video LLM"
    )
