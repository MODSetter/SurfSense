...
async def index_zendesk_tickets(
    ...
    # Log task start
    log_entry = await task_logger.log_task_start(
        task_name="zendesk_tickets_indexing",
        source="connector_indexing_task",
        message=f"Starting Zendesk tickets indexing for connector {connector_id}",
        metadata={
            "connector_id": connector_id,
            "user_id": str(user_id),
            "start_date": start_date,
    ...
    try:
        ...
        zendesk_api_token = connector.config.get("ZENDESK_API_TOKEN")
        ...
        logger.info(f"Starting Zendesk indexing for connector {connector_id}")
        ...
        try:
            ...
            await task_logger.log_task_failure(
                log_entry,
                f"Failed to get Zendesk tickets for connector {connector_id}",
                str(e),
                {"error_type": "TicketFetchError"},
            )
            logger.error(f"Failed to get Zendesk tickets: {e!s}", exc_info=True)
            return 0, f"Failed to get Zendesk tickets: {e!s}"

        if not tickets:
            await task_logger.log_task_success(
                log_entry,
                f"No Zendesk tickets found for connector {connector_id}",
                {"tickets_found": 0},
            )
            logger.info("No Zendesk tickets found to index")
            return 0, "No Zendesk tickets found"
        ...
        for ticket in tickets:
            try:
                ...
                content_hash = generate_content_hash(
                    f"{ticket_subject}{ticket_description}", search_space_id
                )
                ...
                raw_chunks = await asyncio.to_thread(
                    config.chunker_instance.chunk, ticket_description
                )
                ...
                document = Document(
                    search_space_id=search_space_id,
                    title=f"Zendesk - {ticket_subject}",
                ...
                logger.error(
                    f"Error processing Zendesk ticket {ticket.get('id')}: {e!s}",
                    exc_info=True,
                )
                documents_skipped += 1
                continue
        ...
        await task_logger.log_task_success(
            log_entry,
            f"Successfully completed Zendesk indexing for connector {connector_id}",
            {
                "documents_indexed": documents_indexed,
                "documents_skipped": documents_skipped,
            },
        ...
        await task_logger.log_task_failure(
            log_entry,
            f"Database error during Zendesk indexing for connector {connector_id}",
            str(db_error),
            {"error_type": "SQLAlchemyError"},
        )
        logger.error(
            f"Database error during Zendesk indexing: {db_error!s}", exc_info=True
        )
        return 0, f"Database error: {db_error!s}"
    except Exception as e:
        await session.rollback()
        await task_logger.log_task_failure(
            log_entry,
            f"Failed to index Zendesk tickets for connector {connector_id}",
            str(e),
            {"error_type": type(e).__name__},
        )
        logger.error(f"Failed to index Zendesk tickets: {e!s}", exc_info=True)
        return 0, f"Failed to index Zendesk tickets: {e!s}"
