...
- Pin CPython 3.12, the same minor the backend uses, from the python-build-standalone release. One asset per `linux-x64`, `darwin-arm64`, `win32-x64`; there is no Intel Mac build ([ADR 0021](../../../adr/0021-no-intel-mac-build.md)). Record the URL and sha256 in the fetch script, the way `fetch-llamacpp.mjs` records llama-server.
...
- `checkConfiguration()` fails when any of the three targets lacks a 64-hex sha256. That test runs with no network.
