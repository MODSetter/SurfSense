...
- **Memory is checked before anything loads.** The worker compares the operating system's available memory with the model's peak measured while voicing, from its entry, plus 1 GiB, and refuses short of it: "Voicing needs about 2.6 GB free; this computer has 1.1 GB." audio.cpp's own guard counts only the file it reads.
