...
- **Memory is checked before anything loads, twice.** The worker compares the operating system's available memory with the model's peak measured while voicing, from its entry, plus 1 GiB, and refuses short of it: "Voicing needs about 2.6 GB free; this computer has 1.8 GB. Supertonic 3 needs about 1.5 GB." The second sentence names the first other curated audio model, in the manifest's order, that would fit, and is left out when none would. It checks first, before the chat model drafts anything, so a machine that cannot voice the episode does not spend minutes writing it. It checks again at voicing, because the chat model's own memory may have changed the answer. audio.cpp's own guard counts only the file it reads.
