...
- At startup, a chosen model whose connection cannot list its models, because the machine is offline or the endpoint refuses the key, stays chosen: the dashboard opens with sending disabled and asks to reconnect the provider, rather than failing to start ([`app-bootstrap.tsx`](../../surfsense_local/frontend/src/app/app-bootstrap.tsx)).
