...
- Ciphertext this install's secret cannot open, after a new secret was minted or a backup was restored on another machine, raises `UnreadableSecretError`. The API answers any route that hits it with `409` and code `unreadable_secret`: the request is valid, the server is healthy, and the stored key is what has to be entered again. At startup, a selected model whose check fails, for that or any other reason, is left unselected and the app opens without it ([`app-bootstrap.tsx`](../../surfsense_local/frontend/src/app/app-bootstrap.tsx)); a notice above the composer names the reason from the error's code and opens Settings ([`model-issue-notice.tsx`](../../surfsense_local/frontend/src/features/chat/model-issue-notice.tsx)). The exception is `egress_disabled`: a consent not yet given, not a broken model. The model stays selected and the composer is held, with a notice for that model only (its connection and name) that names the host; **Allow…** opens the egress consent dialog, and **Open settings** opens Settings › Network. A send never raises the dialog. The dashboard checks the selected model again when it changes, when Settings closes and after Allow, so switching to a model on an allowed or loopback host clears the notice ([`model-issue.ts`](../../surfsense_local/frontend/src/features/chat/model-issue.ts)).
