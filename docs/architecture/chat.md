...
- A citation carries `source_id`, `chunk_id`, `document_id`, `start_line`, `end_line` and the document's `title`. An assistant turn's stored `content` is `{"text", "citations"}`, plus `"reasoning": {"text", "duration_ms"}` when the model reasoned; a user turn's is `{"text"}`, or `{"text", "citations": []}` when it was imported.
...
- From send until the first token, the assistant turn shows "Thinking…" as a status, so a model load or a long prompt is never a blank reply. A thinking model's trace then streams open above the answer, in a box 12rem tall that follows the newest line until the reader scrolls up, and folds to "Thought for N seconds" when the answer starts; the person's own open or close wins until the next reply ([`reply-thinking.tsx`](../../surfsense_local/frontend/src/features/chat/reply-thinking.tsx)). The trace rides in the message's `metadata.custom`, beside the citations, not as a content part, so Copy takes the answer alone.
...
- The trace renders as plain text, not markdown, and there is no switch to turn thinking off for answers.
