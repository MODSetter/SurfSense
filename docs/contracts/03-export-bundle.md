...
- Backend: the exporter's output passes `check-export-sample.py`; a pending document appears in `skipped`, not `documents`; a message with three citation markers exports with none and three titles.
- App: the zipped fixture imports to two workspaces, five documents, two threads; `index.md` and `log.md` are not documents; a manifest with `../` is rejected before any file is written; importing twice yields no new rows.
