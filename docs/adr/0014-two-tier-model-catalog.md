...
- **Status:** Accepted; the `rank` decision is superseded by [ADR 0026](0026-curated-order-is-list-position.md), and the two egress destinations by [ADR 0027](0027-egress-consent-per-host.md)
- **Date:** 2026-09-19
...
- Two tiers. The curated manifest ([`catalog/curated-models.json`](../../surfsense_local/backend/modules/llm/catalog/curated-models.json)) is the offline product and ships frozen. Hugging Face search is a network feature, and it is absent, not degraded, when egress is off.
...
- Install ids are opaque and minted by the server, for both tiers. The renderer sends a `catalog_id`, and whether to select the model, never a repo or file name ([`catalog/search/tickets.py`](../../surfsense_local/backend/modules/llm/catalog/search/tickets.py)).
- One denylist, keyed by both the GGUF architecture and the repo's pipeline tag, refuses what cannot hold a conversation ([`catalog/search/not_chat.py`](../../surfsense_local/backend/modules/llm/catalog/search/not_chat.py)) (recorded 22 Sep 2026, replacing a generated allowlist).
