...
- Install ids are opaque and minted by the server, for both tiers. The renderer sends a `catalog_id`, and whether to select the model, never a repo or file name (now [`catalog/local/install/tickets.py`](../../surfsense_local/backend/modules/llm/catalog/local/install/tickets.py)).
