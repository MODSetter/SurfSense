# =============================================================================
# NeoNote — One-line Install Script (Windows / PowerShell)
#
#
# Usage: irm https://raw.githubusercontent.com/MODSetter/NeoNote/main/docker/scripts/install.ps1 | iex
#
...
#   1. Fresh install        — no prior NeoNote data detected
#   2. Migration from the legacy all-in-one container (NeoNote-data volume)
...
$RepoRaw            = "https://raw.githubusercontent.com/MODSetter/NeoNote/main"
$InstallDir         = ".\NeoNote"
$OldVolume          = "NeoNote-data"
$DumpFile           = ".\NeoNote_migration_backup.sql"
$KeyFile            = ".\NeoNote_migration_secret.key"
...
Write-Step "Downloading NeoNote files"
...
if (($volumeList -split "`n") -contains $OldVolume -and -not (Test-Path $MigrationDoneFile)) {
    ...
    if (Test-Path $DumpFile) {
        ...
        Write-Info "Full extraction log: ./NeoNote-migration.log"
        Write-Host ""

        $migrateScript = Join-Path $InstallDir "scripts/migrate-database.ps1"
        & $migrateScript -Yes
        if ($LASTEXITCODE -ne 0) {
            Write-Err "Data extraction failed. See ./NeoNote-migration.log for details.`nYou can also run migrate-database.ps1 manually with custom flags."
        }
...
if ($MigrationMode) {
    ...
    if (-not $DbUser) { $DbUser = "NeoNote" }
    if (-not $DbPass) { $DbPass = "NeoNote" }
    if (-not $DbName) { $DbName = "NeoNote" }
    ...
    $restoreErrFile = Join-Path $env:TEMP "NeoNote_restore_err.log"
    Push-Location $InstallDir
    ...
    if ($fatalErrors.Count -gt 0) {
        ...
        Write-Warn "If NeoNote behaves incorrectly, inspect manually."
    } else {
    ...
    Write-Step "Starting all NeoNote services"
    Push-Location $InstallDir
...
if ($SetupWatchtower) {
    ...
    if ($wtState -eq "true") {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Ok "Watchtower started - labeled NeoNote containers will auto-update."
        } else {
...
Write-Host "         AI Research Hub for Teams  [$versionDisplay]" -ForegroundColor Yellow
