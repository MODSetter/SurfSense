...
from modules.llm.catalog.local.engines.audiocpp.audio_folder.espeak import (
    Espeak,
    espeak_session_options,
)
...
def write_server_config(
    folder: Path, installed: Sequence[InstalledAudio], espeak: Espeak | None = None
) -> None:
    ...
    payload = {
        ...
        "lazy_load": True,
        "models": [_entry(model, espeak) for model in installed],
    }
...
def _entry(model: InstalledAudio, espeak: Espeak | None) -> dict:
    entry: dict = {
        "id": model.model_id,
        "family": model.family,
    ...
    if options := espeak_session_options(model.family, espeak):
