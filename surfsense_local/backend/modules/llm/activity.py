...
def model_key(
    ...
    return provider, str(connection_id or ""), name
...
class ModelActivity:
    ...
    def __init__(self) -> None:
        ...
        self._uses: dict[tuple[str, ...], int] = {}
        self._deleting: set[tuple[str, ...]] = set()

    async def acquire_use(self, key: tuple[str, ...]) -> None:
        async with self._lock:
            if key in self._deleting:
                raise ModelBusyError("model is being deleted")
            self._uses[key] = self._uses.get(key, 0) + 1

    async def release_use(self, key: tuple[str, ...]) -> None:
        async with self._lock:
    ...
    @asynccontextmanager
    async def deleting(self, key: tuple[str, ...]) -> AsyncIterator[None]:
        async with self._lock:
