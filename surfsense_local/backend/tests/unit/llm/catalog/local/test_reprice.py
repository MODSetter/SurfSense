...
def service(tmp_path: Path) -> LocalCatalogService:
    ...
    models.mkdir()
    return LocalCatalogService(load_local_manifest(), models, tmp_path / "lib")
...
def test_a_vision_model_names_its_projector_and_reserves_room_for_it(
    ...
    a_model(tmp_path / "Qwen3-VL-Q4_K_M.gguf")
    projector = tmp_path / projector_filename("Qwen3-VL-Q4_K_M")
...
def test_a_lone_projector_is_never_attached_to_a_model_beside_it(
    ...
    """Some other model's projector, or none of this model's: the pairing is
    recorded at install or named after the model, never guessed from a folder."""
    a_model(tmp_path / "Qwen3-8B-Q4_K_M.gguf")
    a_projector(tmp_path / "mmproj-F16.gguf")
    service = LocalCatalogService(load_local_manifest(), tmp_path, tmp_path)
...
def test_a_projector_named_anything_is_not_offered_as_a_model(tmp_path: Path) -> None:
    """Asked of the header, not the name."""
    a_projector(tmp_path / "vision-half-BF16.gguf")
    a_model(tmp_path / "Qwen3-8B-Q4_K_M.gguf")
    service = LocalCatalogService(load_local_manifest(), tmp_path, tmp_path)
