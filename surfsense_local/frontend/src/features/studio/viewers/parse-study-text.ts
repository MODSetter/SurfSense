// Ported from surfsense_web's study-text/parse-text.ts: splits generated
// study content (quiz/flashcards) into plain-text and LaTeX segments. The
// backend prompt (see quiz/pipeline.py) tells the model to mark math with
// \(...\) (inline) and \[...\] (display) — not $...$/$$...$$, which would
// collide with literal dollar amounts ("$5 billion") that this content
// often contains.
...
export function parseStudyText(value: string): StudyTextSegment[] | null {
  ...
  while (index < value.length) {
    ...
    if (index > textStart) {
      segments.push({
        type: "text",
        value: value.slice(textStart, index),
        offset: textStart,
      })
    }
