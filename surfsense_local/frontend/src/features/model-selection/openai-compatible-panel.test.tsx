...
describe("OpenAI-compatible connections", () => {
  it("offers a model only for the role it is classified for", async () => {
    ...
    await user.click(
      await screen.findByRole("button", { name: "Browse models" })
    )
    await screen.findByText("gpt-image-1")

    const roleButtons = (name: string) => {
      const row = screen.getByText(name).closest("li")
      if (!row) throw new Error(`no row for ${name}`)
      return {
        image: row.querySelector<HTMLButtonElement>(
          '[aria-label="Assign as image"]'
        ),
        chat: row.querySelector<HTMLButtonElement>(
          '[aria-label="Use for chat"]'
        ),
      }
    }

    const image = roleButtons("gpt-image-1")
    expect(image.image?.disabled).toBe(false)
    expect(image.chat?.disabled).toBe(true)

    const chat = roleButtons("gpt-4o-mini")
    expect(chat.image?.disabled).toBe(true)
    expect(chat.chat?.disabled).toBe(false)

    const neither = roleButtons("whisper-1")
    expect(neither.image?.disabled).toBe(false)
    expect(neither.chat?.disabled).toBe(false)
  })
  ...
  it("loads and searches models only after opening a connection", async () => {
    ...
    const fetchMock = vi.fn(
      async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (
          path === "/llm/connections/2/chat-test" &&
          init?.method === "POST"
        ) {
          return Response.json({ reply: "Yes, I can answer." })
        }
        return Response.json({ detail: "not found" }, { status: 404 })
      }
    )
    ...
    // Undeclared capability: the model answers once before it takes the role.
    expect(screen.getByText(/chat support\s+is unconfirmed/s)).toBeTruthy()
