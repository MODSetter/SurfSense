...
function Explanation({
  ...
  if (model.capability_source === "unknown") {
    return image ? (
      <>
        This endpoint does not publish capabilities, so image support is
        unconfirmed. It must implement{" /images/generations "}or{" /images"}.
        Testing runs real inference and may cost money.
      </>
    ) : (
      <>
        This endpoint does not publish capabilities, so chat support is
        unconfirmed. Testing sends one short prompt and may cost money.
      </>
    )
  }
  return image ? (
    <>
      Image support is confirmed. Testing runs real inference and may cost
      money.
    </>
  ) : (
    <>
      Chat support is confirmed. Testing sends one short prompt and may cost
      money.
    </>
  )
}
      ...
      <Dialog open onOpenChange={(open) => !open && onClose()}>
        <DialogContent className="select-none">
          ...
          {previewUrl ? (
            <img
              src={previewUrl}
              alt={`Test generated by ${model.name}`}
          ...
          <DialogFooter>
            ...
            <Button variant="outline" disabled={testing} onClick={runTest}>
              ...
              {modelType === "image_gen" ? "Test image" : "Test chat"}
            </Button>
            <Button
