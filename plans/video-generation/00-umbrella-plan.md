...
- **Config (+):** `VIDEO_SANDBOX_RENDERING_ENABLED` in `app/config` + `surfsense_backend/.env.example` (default `FALSE`). Removed again in Phase 8.
...
- **Backend (+), additive touches to shared Tier A:** the flag branch in `deliverables/tools/index.py::load_tools`; HTTP Range on `stream_artifact_file`; `video/mp4` inline in `document_files_routes._is_inline`. No new recorder or serving route — MP4 rides generic `save_artifact` + `stream_artifact_file`.
...
- **Frontend (+):** an MP4 `<video>` component selected by PRIMARY mime (Phase 6, additive — legacy player retained through the transition).
