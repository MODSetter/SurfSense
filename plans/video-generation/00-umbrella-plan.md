...
- **Governing timeout is per-`execute`.** A single render must fit `SANDBOX_OPERATION_TIMEOUT_SECONDS` (per `execute` call), independent of the subagent invoke timeout. Long decks render in segments + `ffmpeg concat`. (Optimization deferred.)
...
- **Signed-URL / CDN delivery.** Backend Range streaming ships first (works on every storage backend incl. local disk, keeps per-request auth). Direct-to-blob signed URLs are a later bandwidth optimization, gated on `backend.supports_signed_urls()`.
