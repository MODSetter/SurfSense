...
- **Sandbox image (+):** Chrome libs (per `/docs/docker`), `remotion`, `@remotion/bundler`, `@remotion/renderer`, `@remotion/media`, `@remotion/media-parser`, baked Chrome Headless Shell, and a system `ffmpeg` (Remotion's bundled ffmpeg renders/muxes; the system binary is for segment concat and verify frame sampling, which don't go through the renderer — Phase 1 §2).
...
- **Backend (+), net-new modules (Tier B-new, no legacy imports):** `deliverables/tools/synthesize_narration.py`; `artifacts/verification/formats/video.py` (+ `.mp4` entry in `formats/registry.py`, `"video"` on `ReviewKind` in `formats/base.py`); `scripts/backfill_video_mp4.py`.
- **Backend (+), additive touches to shared Tier A:** the flag branch in `deliverables/tools/index.py::load_tools`; a second, video verify pipeline in `verification/service.py` (`_verify_video`, dispatched off the video adapter — frames sampled by sandbox `ffmpeg` mirroring `rasterize_pdf`, then the shared `review_pages`); an explicit `format=verification.format` on the `save_artifact_tool` persistence call so a verified MP4 stores as `ArtifactFormat.VIDEO`, not the `.mp4` suffix; HTTP Range on `stream_artifact_file`; `video/mp4` inline in `document_files_routes._is_inline`. No new recorder or serving route — MP4 rides generic `save_artifact` + `stream_artifact_file`.
...
- **Frontend (+):** a plain, lazy (`preload="none"`) MP4 `<video>` rendered **inline in the `save_artifact` chat card** — selected by PRIMARY `video/mp4` mime, playing where it was produced (like podcasts), not in the panel; seeking rides the Phase-5 Range endpoint (Phase 6, additive — legacy player retained through the transition).
