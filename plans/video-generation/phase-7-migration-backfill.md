...
- **Structural verify.** Backfill runs the **same structural-only verify as the live path** (Phase 4 — ffprobe: 1920×1080, duration > 0, video + audio stream), so there is no quality-gate special-casing to reason about: it faithfully reproduces already-shipped content, and the structural gate is exactly what a re-render of known-good decks should clear. It produces the byte-bound receipt `save_artifact` requires. Genuine failures (won't bundle, audio missing) route to Freeze, not a quality rejection.
