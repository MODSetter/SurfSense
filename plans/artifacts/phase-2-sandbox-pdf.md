...
- Subagent system prompt gains format-selection guidance (master spec §6.2); `generate_report` and `generate_resume` are unregistered now, while their historical data and renderers remain until phase 4.
...
- Regression: neither prompt, tool catalog, nor deliverables registration exposes `generate_report` or `generate_resume`; historical rendering/data remains untouched until phase 4.
