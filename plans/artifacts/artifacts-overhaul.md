...
**Status:** Phases 1–2 implemented on this branch (phase 2 carries a verified spike result and its implementation corrections); phase 3 is next; phases 3–6 planned
...
| `save_artifact` | `(path, source_path, title, markdown_representation, preview_path?, document_id?) → §3.1 payload` | Write-through persist: reads bytes from the sandbox, creates `Document` + `DocumentFile`(s) in one transaction, returns the contract payload. `source_path` is required beside `path` for generated files — the deliverable without the thing that made it is an artifact that can only ever be replaced, never edited (§4.1). **Also the verification gate** — refuses a primary file whose mtime is newer than the most recent verification *of the kind that file's format requires*, so the §6.3 loop is an invariant of the tool rather than a promise repeated in every skill's prose. Page-based formats require a vision inspection and cannot be saved on a structural script alone, since what fails in them is layout and only a rendered page shows it; every other format requires the §7.1 sentinel, whose path must name the primary or its preview so that an assertion script cannot vouch for a file nobody checked. The two kinds are recorded in separate ledgers, because freshness is a ledger file's own mtime and one shared file would let a checker re-run after the vision pass erase it. Which formats are page-based is keyed on the suffix (`.pdf`, `.docx`, `.pptx`) rather than asked of `mimetypes`, which cannot name the OOXML formats in this runtime for two independent reasons: CPython 3.12's built-in table has no entry for them (they arrive in a later version), and `python:3.12-slim` omits the `/etc/mime.types` that would otherwise supply one. `guess_type("out.docx")` is therefore `None` in production, so a gate that asked it would wave every docx straight through while passing review and passing tests on any machine that happens to have that file. `python-magic`, already a dependency, *does* identify OOXML correctly from the bytes — the reason the check does not use it is ordering, not capability: the gate runs before `save_artifact` pulls the file out of the sandbox, so that a refusal costs one `stat` rather than a full transfer of a file the tool is about to reject. "Could not verify" is not "did not verify": when no vision model is configured or premium credit runs out mid-loop, the save proceeds with the reason recorded in `document_metadata`. The `markdown_representation` then reaches the search index per §4.4 — written into the turn's working copy on git workspaces (committed + indexed by the KB pipeline), indexed directly on legacy ones. Markdown artifacts pass content directly with no `path`. |
...
3. Convert to PDF if not already PDF: soffice --headless --convert-to pdf resume.docx
   (§7.1 conventions: private profile, explicit --outdir, stale PDF deleted first, result checked)
4. Rasterize: pdftoppm -jpeg -r 100 resume.pdf page
...
8. Only then: save_artifact(path=resume.docx, source_path=resume.js, preview_path=resume.pdf, …),
   which rejects the save if the file changed after step 5-6, or if those steps never ran:
   for a page-based format, step 2 alone does not open the gate.
   (Files are named for the deliverable, never out.<ext> — §7.1: the basename is the download name.)
```

**Programmatic** — `xlsx`: no rasterizing and no vision call. Recalculate headless, read the expected cells back, assert, and print the §7.1 sentinel — with no vision call in this shape, that token is the *only* thing that reaches the gate, and a script that asserts and exits quietly is a save the tool will refuse. See §7.1 — the recalculated file is the file saved. The two shapes are not exclusive: the visual skills run the measurable checks of step 2 as well, which is the programmatic shape applied to what pixels would answer more expensively.
...
Four skills, each a directory under **`{skills_root}`** — `docker/sandbox/skills/` in the repo, copied into the sandbox image and read by the agent at `/opt/skills/` (`docker/sandbox/Dockerfile`: `COPY skills /opt/skills`) — as `{skills_root}/{format}/SKILL.md` + `scripts/`, authored fresh (Anthropic's skill *files* are license-restricted — "Proprietary, see LICENSE.txt" — so we write our own against the same toolchain, which their docs openly describe). One root, two names: the repo path is what CI's roster check and the image build read, `/opt/skills/` is what every path in a skill body and a prompt says. Nothing about format skills routes through `main_agent/skills/` (§2.3).

| Skill | Create with | Verify with | Notes |
|---|---|---|---|
| `pdf` | reportlab / weasyprint (Python) | pypdf measurements → pdftoppm → `inspect_sandbox_images` | **The default for prose deliverables** — resumes, reports, letters, one-pagers (§6.2). Measurable defects first, then every page reviewed and compared (§6.3) |
| `docx` | `docx` (npm, Node) | soffice → pdf → pdftoppm → `inspect_sandbox_images` | Same loop as `pdf`, plus the conversion hop below. Selected only when the user asks for Word by name (§6.2). Encode the known footguns: DXA table widths, `ShadingType.CLEAR`, numbering for bullets, TOC outline levels *if* a TOC was asked for at all (phase 3 §2.2), tab stops over PositionalTab |
...
- **Shape.** Frontmatter triggers (Level 1, §6.2), a body under ~500 lines, and the skill's **own** `{skills_root}/<name>/scripts/` — a copied rasterizer is cheaper than an indirection every skill depends on, and no shared script directory competes with real skills under `/opt/skills/`. Each body states which verification shape it uses (§6.3) and then covers only how to render evidence for its format and what to look for in it. None restates "never save before verifying" and none describes revision: `save_artifact` holds the first invariant and `load_artifact_source` the second, both format-blind, so a skill inherits them by existing.
- **Verification is claimed with a sentinel, not an exit code.** A structural or assertion script counts as a verification only if its output carries a line that *opens* with `SURFSENSE_VERIFIED:` and names the path it checked — `execute` records a structural verification when it sees such a line in a clean run and at no other time (`.../deliverables/tools/sandbox.py`; `pdf`'s `check_pdf.py` prints it as its last line). Exit code alone cannot carry the meaning, because *every* successful `execute` exits clean, including the one that generated the file. A script that asserts hard and exits 0 silently leaves the gate exactly where it was, and `save_artifact` then refuses the save with the model having done the work. Both halves of the shape are load-bearing: the token must open the line, or a run that merely mentions it — this file being printed, say — claims a verification nobody performed; and the path is checked against the primary and its preview by filename, or a script can vouch for whatever was convenient rather than for the artifact being saved.
...
- **Two-hop formats must prove the middle link themselves.** The gate compares the primary's mtime against the ledger for the kind the format requires (`.../deliverables/tools/verification.py`); for `pdf` the inspected pages come straight off the primary, so it is sound. For docx and pptx the chain is `<name>.docx → <name>.pdf → page-*.jpg` and nothing validates the middle hop: a failed re-conversion leaves the *previous* PDF sitting there, the model inspects last generation's pages, and the gate still reports CURRENT because the primary is older than that inspection. So the conversion step **deletes the intermediate PDF and the page renders before regenerating them, then asserts the new PDF exists, is non-empty, and is newer than the primary** (`pdf`'s `render_pages.sh` already sets the precedent with its `rm -f` of stale pages). Stale evidence is the one failure the loop cannot see for itself.
- **`soffice` needs three things, and omitting any of them fails quietly.** Give each conversion a private profile (`-env:UserInstallation=file:///tmp/soffice-<unique>`): a second invocation against the default profile dies on the lock the first one left, which is every reconvert in a fix-and-retry loop and every concurrent session sharing the image. Pass an explicit `--outdir` rather than trusting the working directory. And check the output afterwards, because soffice exits 0 having produced nothing often enough that its status is not evidence — which is the same check the two-hop rule above already demands.
- **Name the output after the deliverable, never `out.<ext>`.** The download filename the user gets is the sandbox basename (`PurePosixPath(path).name` in `save_artifact`), not the title — so `out.docx` on disk is `out.docx` in their downloads folder, which is not what §3.1's example promises. A skill writing `indian-history.docx` (with `indian-history.js` and `indian-history.pdf` beside it) fixes that for free and for every format, where deriving a filename from the title inside the tool would be a second naming authority to keep in agreement with the first. Applies to `pdf` retroactively.
...
- Skills at `/opt/skills/`, copied in from `{skills_root}` (§7.1)
...
| DOCX / PPTX with no preview file (revision omitted it, or conversion failed) | "Can't preview this here" message | The real .docx/.pptx | Preview-absent branch in `PdfPreviewViewer` |
...
| 3 — `docx` | [`phase-3-docx.md`](./phase-3-docx.md) | `docx` skill (first consumer of the §7.1 skill conventions), preview-PDF pairing exercised end to end, `PdfPreviewViewer` | A Word doc generates, verifies, and renders through its preview PDF; two blobs out, both purged on delete |
| 4 — `pptx` | [`phase-4-pptx.md`](./phase-4-pptx.md) | `pptx` skill (incremental per-slide verification), one `PdfPreviewViewer` registry line | A deck generates, verifies per-slide and together, renders; the phase costs one skill + one registry line + one integration-test case |
| 5 — `xlsx` | [`phase-5-xlsx.md`](./phase-5-xlsx.md) | `xlsx` skill (programmatic verification, mandatory recalc), `XlsxViewer` (native grid), unviewable/download-card polish, public-chat artifact rendering | All four formats per the §8.3 matrix; unknown formats degrade to the card with no code changes; no path routes to legacy tools |
