...
| 3 — `docx` | [`phase-3-docx.md`](./phase-3-docx.md) | Office-skill conventions, `docx` skill, preview-PDF pairing exercised end to end, `PdfPreviewViewer` | A Word doc generates, verifies, and renders through its preview PDF; two blobs out, both purged on delete |
| 4 — `pptx` | [`phase-4-pptx.md`](./phase-4-pptx.md) | `pptx` skill (incremental per-slide verification), one `PdfPreviewViewer` registry line | A deck generates, verifies per-slide and together, renders; the phase costs one skill + one registry line + one harness row |
