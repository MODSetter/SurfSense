...
3. **Stream:** `StreamingResponse` over `open_document_file_stream()` with headers:
   ...
   - `Content-Disposition`: `inline` only for MIME types on the inline allowlist (`application/pdf`, `text/plain`, and `image/*` **except `image/svg+xml`**); `attachment` for everything else. Always includes the original filename. This is the stored-XSS guard: an agent-generated `.html` or `.svg` downloads instead of executing on our origin. SVG is carved out of `image/*` precisely because it is the one image format that carries script, and `nosniff` does not help — the browser is honoring a `Content-Type` we chose.
