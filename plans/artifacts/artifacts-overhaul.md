...
**Status:** Phases 1–4 implemented on this branch; phase 5 is next; phases 5–6 planned
...
**Programmatic — `xlsx`:** no conversion to PDF, no rasterizing, no vision call. The adapter reads the workbook's cells back and the service issues a receipt whose visual verdict is *not applicable to this format*. Same receipt shape either way, so the gate needs no format branch and no skill needs prose about which shape it is in. The page-based formats run step 1 as well: it is this shape applied to everything pixels would answer more expensively. The adapter carries the difference as a shape rather than a flag — its rendered policy (conversion, rendered thresholds, exact-count expectation, review kind) is *absent* rather than set to disabling values, so the service branches once after the structural check and a format that is never rasterized cannot also declare how it should be looked at.

**Verification inspects; it never rewrites.** An earlier revision had the service recalculate the xlsx headless *in place* and hash what it produced, because openpyxl writes formulas with no cached values while the grid viewer renders cached values (§8.2). That was the wrong address for the work. A verification step that mutates the deliverable is an invariant every later format inherits, it forces an async session-carrying hook into an adapter contract that is otherwise a pure `bytes -> StructuralCheckResult`, and a failure mid-transform leaves a file that is neither generation. The same guarantee is available one layer earlier and in a checkable form: the skill writes each formula's computed result into the cell cache as it authors (§7.1), and the adapter rejects a workbook whose formula cells carry none. Completeness becomes a **property of the file** rather than a process the service performed — unskippable, non-mutating, and identical for every format that follows. Where a future format genuinely needs a converter run over it, that belongs in its skill's script, in the sandbox, before `verify_artifact` is called, alongside every other piece of generation.

**Every format resolves, including the ones nobody wrote.** `get_format_adapter` is total: an unregistered suffix returns a generic adapter — non-empty, within the size cap, no rendered policy, canonical MIME `application/octet-stream` — so `verify_artifact` works on anything the agent produces and `save_artifact` keeps one universal rule, *no binary is saved without a receipt*, with no format branch in the persistence layer (§1.2, principle 4). A registry that raised instead put format knowledge in `save_artifact`, where a `.csv` the agent wrote with pandas could not be saved at all. Guessing the type with `mimetypes` was rejected in favour of octet-stream: a MIME comes from an adapter or it does not come, which keeps the viewer registry from matching a guess and keeps the download route's inline allowlist (PDF only, an explicit XSS boundary) away from a type it might render. An unknown format degrades to the download card by resolving correctly, not by failing gracefully.
...
*Generation* is the one asymmetry: independent units such as slides and worksheets can be built incrementally with local source assertions, while a flowing PDF cannot — there is no "page 2" until the whole document renders, pagination is emergent, and editing anything above a break moves everything below it. Rendered verification still runs over the complete artifact once: calling it after each slide would repeatedly convert and review the whole prefix until content-addressed skipping exists (§12).
...
| `pptx` | python-pptx | `verify_artifact` — exact source/rendered count, then slide-aware visual review | Slides are built incrementally with local assertions, then the complete deck is rendered and verified once |
| `xlsx` | XlsxWriter | `verify_artifact` — cells read back, no vision call | Verified programmatically (§6.3). Formulas are written **with their computed result** in the cell cache (`write_formula(…, value)`), while XlsxWriter separately sets the recalculate-on-load flag Excel and LibreOffice honour, so a stale cached value self-corrects the moment the file is opened. The grid viewer renders cached values and openpyxl writes a formula *or* a value, never both, so a workbook authored without them renders blank formula cells (§8.2) — the adapter rejects one that lacks them, making the convention enforced rather than remembered |

Adding a format later = one skill body + one format adapter in the verification service (§6.3) + a sandbox image rebuild + (optionally) one viewer-registry entry. The adapter owns the format's suffix, canonical stored MIME, structural checker, and either a rendered policy (conversion, rendered-check parameters, visual-review kind) or none at all — which is what says it is verified programmatically — so verification and persistence cannot disagree about what a format is. Skipping the adapter is also a supported outcome rather than a failure: an unregistered suffix resolves to the generic one (§6.3), which is how a format nobody planned for still verifies, persists, and downloads. The adapter is the item earlier revisions of this spec promised away as "no backend changes", and the promise was worth less than it sounded: the checks it holds were always going to be written, and the alternative address for them was a script in the image that nothing could unit-test and every skill had to invoke correctly by hand.
...
- **The roster line's wording is a test contract, not prose.** The Level 1 roster (§6.2) is held against the installed frontmatter by `tests/unit/sandbox/test_deliverables_skill_roster.py`, which extracts what the prompt advertises with the regex ``Available format skill: `([^`]+)` `` — one match per skill. The decision is to repeat the whole phrase per line (``- Available format skill: `docx` — …``). It is mildly redundant prose, but it leaves one frozen test contract for every later format instead of changing the parser as the roster grows.
...
- Python 3.12 + `xlsxwriter`, `openpyxl`, `python-pptx`, `reportlab`, `weasyprint`, `pypdf`, `pandas`, `matplotlib` — `xlsxwriter` authors workbooks (§7.1, cached formula results); `openpyxl` stays because it reads them and `pandas` reaches for it
...
- `XlsxViewer` renders the **primary** file natively — no preview blob exists for xlsx: fetch `content_url` (existing authenticated-fetch pattern, ETag-cached), parse in-browser with **ExcelJS** (MIT — values, fills, fonts, borders, merged ranges, column widths, sheet list), format display text with **`ssf`** (Apache-2.0 — turns raw `10413` + `"$#,##0.00"` into `"$10,413.00"`, which ExcelJS itself cannot do: it exposes `numFmt` as a raw string and ships no engine for it), and render a read-only grid with column letters, row numbers, and sheet tabs. Row-capped for large sheets ("showing N of M rows — download for full data"); parse failure or oversize falls through to the unviewable state. Fidelity boundary: cell data and styling render; charts, conditional-formatting rules, and pivot tables do not — it is a grid, not an Excel emulator (Claude's viewer shares this boundary). Three build constraints go with it. **ExcelJS lives behind one module** — a `parseWorkbook(bytes): WorkbookView` boundary the component imports instead of the library, which is both the only part of this viewer a unit test reaches cheaply (§8.3) and what keeps a lighter parser a one-file swap. **Size is checked before the viewer mounts** (`size_bytes` is already in the manifest), because rows must be parsed before they can be capped and `xlsx.load()` is CPU-bound enough to jank the main thread. **No virtualizer and no grid library** — a capped plain table with `content-visibility: auto` on rows is enough, and `@tanstack/react-table` is a headless *data* table (sorting, filtering, pagination), the wrong shape for a fixed spreadsheet grid.
- All viewers lazy-loaded via `next/dynamic` (pdf.js and ExcelJS stay out of the main bundle).
- Rationale for the matrix: browsers render PDF natively (pdf.js); client-side office renderers were evaluated and rejected on evidence (mammoth discards layout by design; docx-preview approximates; no credible OSS PPTX renderer; MS Office Online viewer requires public URLs). XLSX preview-as-PDF is likewise rejected — it misrepresents spreadsheets (truncated sheets, invisible formulas) — but a native grid does not: ExcelJS + ssf render what Claude's own xlsx viewer renders. Parser selection was evidence-driven and **re-measured 2026-08-12** against the field that has appeared since (esbuild, minified, `platform=browser`, gzipped): **ExcelJS chosen** at 270 KB (MIT, parses styles in-browser), over `xlsx-js-style` 347 KB (npm release frozen at 2022-04), **SheetJS CE** 335 KB (cell styling is Pro-only — maintainers confirm CE's `cellStyles` reads only row/column metadata), `@extend-ai/react-xlsx` 723 KB plus a 4.3 MB WASM package (four months old, bundles `regl` and four `d3-*` packages to draw charts this viewer does not render), `xlsx-preview` 275 KB (ExcelJS plus an HTML-string renderer, and its repository 404s), `@file-viewer/renderer-spreadsheet` (seven weeks old, canvas grid — no text selection, no copy, no find-in-page), and **Univer** (xlsx import is a commercial Pro plugin requiring their server, watermarked and size-capped unlicensed). Lighter is only available by giving something up: `read-excel-file` is 14 KB with no styles, `fflate` + `ssf` is 10.6 KB with a parser to own. ExcelJS being dormant (4.4.0 released 2023-10, last commit 2025-01, 800 open issues) is a knowingly accepted cost — no live advisory, 13.2M weekly downloads, and it only ever parses workbooks this system generated, which is the long tail those issues are made of; the boundary module above is what keeps the 10.6 KB path open if that changes.
...
| 4 — `pptx` *(shipped)* | [`phase-4-pptx.md`](./phase-4-pptx.md) | Format-general rendered policy, shared OOXML guard, then the `pptx` skill + adapter and one `PdfPreviewViewer` registry line | A deck generates, verifies as a complete slide set with exact count, renders, and downloads as PPTX |
| 5 — `xlsx` | [`phase-5-xlsx.md`](./phase-5-xlsx.md) | Programmatic verification pipeline + total format registry, then the `xlsx` skill + its adapter (cached formula results, no rasterization), `XlsxViewer` (native grid), public-chat artifact rendering | All four formats per the §8.3 matrix; unknown formats degrade to the card with no code changes; no path routes to legacy tools |
...
| Large workbook parsed in the browser (`XlsxViewer`) | Viewer lazy-loaded, so ExcelJS's 270 KB gzipped is never in the main bundle and is downloaded only by someone opening a spreadsheet; size checked from the manifest before the viewer mounts, since a row cap runs after the parse it would need to bound; row-capped render with a "download for full data" notice <!-- ponytail: row cap, not streamed parsing; ceiling is very large sheets rendering partially, upgrade path is a streaming/worker parse behind the same parseWorkbook() boundary -->; parse failure or oversize degrades to the download card, never an error |
| ExcelJS is dormant (4.4.0 released 2023-10, last commit 2025-01) | Accepted with an exit: MIT, no live advisory, 13.2M weekly downloads, and it only parses workbooks this system generated. All of it sits behind one `parseWorkbook()` module (§8.2), so replacing it with `fflate` + `ssf` (10.6 KB gzipped, both healthy) is one file, not a viewer rewrite |
