...
- **Deletion — one model, both directions.** From the tree: `rm` the artifact's file (agent tools) → the turn commits the removal → the projection deletes the row → FK cascade drops the `DocumentFile` rows → `purge_document_blobs()`. From the UI: deleting the document routes through the store facade's direct-caller adapter (git-KB Phase 7, `delete_documents`), which removes the file from git in the same motion — row and tree never disagree, nothing resurrects. Notes and artifacts alike.
