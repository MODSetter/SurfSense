...
1. Check what needs no eyes, from the file's own bytes: text outside the margins,
   blank pages, unembedded fonts — and per format the defects only its own markup
   shows (percentage table widths, literal "-" bullets, a table with no grid).
...
**Revising enters at the same place**, and nothing about it is per-format: `load_artifact_source(document_id)` first, then edit what comes back rather than rebuild from the markdown representation, then verify and save as usual. The source is whatever that format's generate step wrote — a Node script for docx, Python for pptx or xlsx, HTML for a weasyprint PDF — and `save_artifact` demands it back without knowing or caring which. A skill therefore says nothing about revision and nothing about verification: `load_artifact_source`, `verify_artifact` and `save_artifact` hold those invariants between them, format-blind, so a new format inherits all of it by existing.
...
5. verify_artifact("resume.docx") → OOXML checks, soffice → resume.pdf, pdftoppm → page-1.jpg,
   ...
   (One page, so the window comparison has nothing to compare and says so.)
...
- **Shape.** Frontmatter triggers (Level 1, §6.2) and a body under ~500 lines — no scripts directory beside it, because there is nothing left to put in one (§6.2, Level 3). The body covers how to author well in that format and what its own failure modes look like, and stops there: it does not describe the verification procedure, does not say "never save before verifying", and does not describe revision. All three belong to the format-blind tools every skill calls (`verify_artifact`, `save_artifact`, `load_artifact_source`), so a skill inherits them by existing, and four skills cannot drift into four disciplines.
...
- **The keys are only as good as the stored MIME, which does not come free.** Every row above is matched against `DocumentFile.mime_type` as `save_artifact` recorded it, and in this runtime `mimetypes.guess_type("resume.docx")` is `None` for two independent reasons: CPython 3.12's built-in table carries no OOXML entries (they arrive in a later version), and `python:3.12-slim` ships no `/etc/mime.types` to supply them. The tool falls back to sniffing the bytes, which for OOXML honestly reports the zip container it is — so a docx would store `application/zip`, miss every key in this table, and render the unviewable state for a format the registry explicitly supports. On the deployment image only: any machine with a system `mime.types` file passes review and passes tests. The formats this spec ships therefore register their types explicitly (`mimetypes.add_type`) before the lookup runs, which makes the stored MIME a property of our code instead of the base image's file list.
