...
- **Verification is claimed with a sentinel, not an exit code.** A structural or assertion script counts as a verification only if its output carries the token `SURFSENSE_VERIFIED: <path>` — `execute` records a verification when it sees that token in a clean run and at no other time (`.../deliverables/tools/sandbox.py`; `pdf`'s `check_pdf.py` prints it as its last line). Exit code alone cannot carry the meaning, because *every* successful `execute` exits clean, including the one that generated the file. A script that asserts hard and exits 0 silently leaves the gate exactly where it was, and `save_artifact` then refuses the save with the model having done the work.
...
- **Two-hop formats must prove the middle link themselves.** The gate compares the primary's mtime against a single ledger (`.../deliverables/tools/verification.py`); for `pdf` the inspected pages come straight off the primary, so it is sound. For docx and pptx the chain is `<name>.docx → <name>.pdf → page-*.jpg` and nothing validates the middle hop: a failed re-conversion leaves the *previous* PDF sitting there, the model inspects last generation's pages, and the gate still reports CURRENT because the primary is older than that inspection. So the conversion step **deletes the intermediate PDF and the page renders before regenerating them, then asserts the new PDF exists, is non-empty, and is newer than the primary** (`pdf`'s `render_pages.sh` already sets the precedent with its `rm -f` of stale pages). Stale evidence is the one failure the loop cannot see for itself.
