...
- Per-skill integration test, parameterizing the phase 2 §2.7 harness rather than rebuilding it: generate → verify loop ran (trace shows page/cell inspection) → §3.1 payload with correct roles → renders per the §8.3 matrix → a later-turn change revises in place, one document, source read back rather than rebuilt. Each format is a row; a format that needs its own file has lost the generality phase 2 built.
