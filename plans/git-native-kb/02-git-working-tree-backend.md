...
- **`path_resolver` path computation retires for flagged document workspaces** — folder walk + collision suffixing → real repo paths (filename rules still reused, C1). The repo keeps `documents/` and `artifacts/` as separate current roots. `GitTreeBackend` mounts only the working copy's `documents/` subtree for generic KB file tools; the dedicated artifact service writes artifact representations under `artifacts/` and owns their identity, API, and lifecycle. Commit projection dispatches the two roots to separate row/chunk domains.
