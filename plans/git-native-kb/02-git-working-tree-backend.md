...
- **`path_resolver` path computation retires for flagged workspaces** — folder walk + collision suffixing → real repo paths (filename rules still reused, C1). **`DOCUMENTS_ROOT` is a virtual-namespace constant and never appears in a repo path** (C1: the tree is the virtual tree *minus* `/documents`). Shipped both ways: `GitTreeBackend` mounts the working-copy root under the name `documents`, so `MultiRootLocalFolderBackend` consumes that first segment, and the editor/upload recorder converts with `path_resolver.to_store_path`.
