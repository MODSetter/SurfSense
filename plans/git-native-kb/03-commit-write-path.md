...
1. In `commit_staged_filesystem_state`: replace the ordered Postgres write pass with a pass that replays the staged ops as `tx.write`/`tx.remove`/`tx.move` inside one `KnowledgeStore.transaction(...)` scope, reading the staged ops from the existing state keys (`files`, `staged_dirs`, `pending_moves`, `pending_deletes`, `pending_dir_deletes`, `dirty_paths`, `doc_id_by_path`, …; see C4). Keep the same ordering and the `_final_path` move-alias resolution.
