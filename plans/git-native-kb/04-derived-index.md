...
> **Shipped.** `app/knowledge_store/indexer.py` plus the Celery wiring; work items 1, 2, 4, 5, 6 are all in. Where the build differs from what was planned here, the plan text says so inline under **Built as**; those notes are the record of what changed and why. Umbrella: [`00-umbrella-plan.md`](00-umbrella-plan.md).
>
...
1. `app/knowledge_store/indexer.py` — `index_revision(workspace_id, revision)` and `reindex(workspace_id)` over one shared `_converge(...)`. Paths come from `list_changes` / `list_paths` (shipped: added/modified/removed + content ids), content from `read_as_of`; each document is upserted then handed to `IndexingPipelineService.index()` wrapped in a synthetic `ConnectorDocument`. Removed paths drop their document row (chunks cascade).
   ...
   **Built as** `index_revision(session, workspace_id)` — no revision parameter, and the session is the caller's. Three deviations, all forced by the code:
...
4. `reindex(workspace_id)` behind a Celery task (mirror `knowledge_store_janitor_task.py`; register in `app/celery_app.py`). Serialize per workspace with an **index lock distinct from the write lock**: the write lock's 30s TTL is sized for a commit, and reusing it would stall agent writes behind embedding calls.
   ...
   **Built as** `workspace_index_lock` beside `workspace_write_lock` over a shared `_workspace_lock(purpose, ttl, wait)`; the write lock keeps its key and its 30s TTL. A contender gives up in 5s and returns rather than queueing, because the holder converges to the current revision anyway and the sweep re-drives whatever it missed. Whole-workspace rebuilds route to the connectors queue; the per-save task stays on the fast queue since search freshness is user-facing.
...
6. **Self-healing drift, not a deploy step.** A new `workspaces.last_indexed_revision` makes `last_indexed_revision != get_current_revision()` a drift predicate; a daily Beat task enqueues `reindex` for drifted flagged workspaces. That one mechanism covers the initial backfill of already-flagged workspaces (they have git content and no index today), a lost Celery task, a crashed worker, and any workspace flagged later. Runbook steps get forgotten; converging systems don't.
   ...
   **Built as** `reindex_drifted_workspaces`, **hourly with a per-run enqueue cap**, not daily. `KNOWLEDGE_STORE_ENABLED` is a process-global env flag rather than per-workspace, so the first sweep after enabling it is a full-fleet backfill; daily plus a cap would take weeks to drain, and hourly also cuts recovery from a lost task from a day to an hour. The cap bounds the fan-out, not the check — the drift check is one HEAD read per workspace, while each task it enqueues embeds. Candidate workspaces come from the store directory listing (`stored_workspace_ids()`), since no column says which workspaces have a repo.
