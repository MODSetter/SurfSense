...
- **The tree has two current projected roots.** A document at virtual `/documents/<folder>/<title>.md` lives at `documents/<folder>/<title>.md`; a generated artifact representation lives at `artifacts/<title>.md`. The leading slash is stripped and the domain root is retained. `/documents/**` projects to `Document`/`Chunk`; `/artifacts/**` projects to the dedicated `Artifact`/`ArtifactChunk` domain by `(workspace_id, path)`. An artifact is never adopted into a document row. Other top-level paths are ignored by convergence.
...
- **Artifacts share convergence, not rows.** `/artifacts/**` is dispatched to artifact row helpers and `ArtifactIndexingService`, which writes `ArtifactChunk`; `/documents/**` continues through document projection and `Chunk`. A full tree run prunes both ownership maps independently. Artifact indexing state is generation-aware, and an artifact is searchable only when its indexed generation is current. Folder reconciliation remains document-only.
- **Git artifact durability precedes convergence.** `save_artifact` commits `Artifact`/`ArtifactFile` inside the tool call and adds only the representation to the private worktree. Later projection or embedding failure records pending/failed indexing and does not roll back that artifact. Non-git workspaces run the artifact indexer in the save transaction instead; an indexing failure rolls back the attempted save and staged file rows.
...
- The document-only `search_chunks` compatibility surface remains available to its legacy REST/report callers. Knowledge-base agent search uses `search_knowledge_base`: one query embedding, document and artifact candidate legs, and one global semantic/keyword rank fusion before source grouping. Artifact candidates come only from current ready generations (`indexed_generation == generation`).
