...
- **(2026-08-06) Generated artifacts are store citizens, and forward-only.** The artifacts overhaul ([`plans/artifacts-overhaul.md`](../artifacts-overhaul.md) §4.4) puts each artifact's markdown representation in the store via the turn's working copy (binaries stay in the blob store — the same split as uploads) with destructive replace-on-revise and no version history. No store or indexer changes needed; the constraint this umbrella inherits is on **future verbs**: the revert verb excludes `generated: true` documents' paths from its inverse diff, and any version-history UI excludes generated documents — an old entry for one is a description whose deliverable no longer exists.
