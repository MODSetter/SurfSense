...
- Desktop build downloads `catalog.json` from `plugin-catalog` the way `fetch-llmfit.mjs` downloads a pinned asset, and electron-builder ships it in `extraResources`. A failed download fails the desktop build. The URL is a constant in that script and the same constant in the app.
...
- `license: paid` consults `modules/license/verify.py`'s stored result. Missing, expired, `trial`, or `clock_untrusted` refuses the install and the run with `license_required`. `free` does not consult it.
...
- Add `plugin_catalog` and `plugin_install` next to `OLLAMA_PULL` in `modules/egress/service.py`, and include them in `list_destinations`, so both appear in Settings → Network, off by default.
...
- A `paid` entry with no license does not insert `installed_plugins`. The same entry with `license: free` does, with no license on disk.
