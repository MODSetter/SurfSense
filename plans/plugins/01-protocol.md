...
For each secret name in the manifest, the app sets `SURFSENSE_PLUGIN_SECRET_<NAME>` to the stored value. Secret names match `^[a-z][a-z0-9_]{0,63}$` so they are environment variables. The app refuses to spawn when a declared secret has no stored value.
...
The plugin's own HTTP is ordinary Python. The app does not see those calls, does not apply the `hosts` list to them, and does not apply Settings → Network to them. `hosts` is what the catalog shows.
