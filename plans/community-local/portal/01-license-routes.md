...
They also bound mail spend and check-out load: one resend is N check-out
calls. There is no licensing quota to protect — Keygen is self-hosted.
...
```
KEYGEN_API_URL=                      # self-hosted CE base, e.g. http://keygen:3000/v1/accounts
KEYGEN_ACCOUNT_ID=                   # generated by us in CE singleplayer, not assigned
KEYGEN_API_TOKEN=
KEYGEN_POLICY_TRIAL=
...
The fakes are what let all of this be tested with no Keygen instance and no
...
Self-hosting makes the **Keygen** half of that gap closable in the same shape,
if it is ever worth the hour: a real instance is just a compose service, so an
opt-in `test_keygen_contract.py` could create a license, check out a file and
verify the signature against the real API rather than trusting
`fake_keygen.py`. On Keygen Cloud that test was not worth writing — every
license it created would have counted as a billable active licensed user for
90 days, and the count accumulates across runs (see the pivot plan's Licensing
log). Against CE it is free.
