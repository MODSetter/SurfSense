# API — Phase 8: CUDA as an optional second backend

> Owns: the Windows GPU backend payload and multi-device selection. Extends
> [`07-llamacpp-runtime.md`](07-llamacpp-runtime.md), which ships **Vulkan on
> every platform**. Nothing here is required for phase 7 to be complete, and
> nothing here changes application code.
>
> **Status: not scheduled.** This phase exists so the decision is written down
> with its evidence, not so it gets built. It ships only if the measurement in
> **8.0** comes back materially different from the one in the Appendix.
...
Phase 7 ships Vulkan everywhere:
...
| **Device selection** | First device with `type == GPU`. **Never sum** | Specified in [`07-llamacpp-runtime.md`](07-llamacpp-runtime.md) — 7.2, and needed there anyway. What this phase adds is the duplicate case: with both backends loaded the same physical card appears twice, both typed `GPU`. |
...
**`select_device()` is specified in [`07-llamacpp-runtime.md`](07-llamacpp-runtime.md)
— 7.2**, and is required there regardless of whether this phase ever ships: the
...
`select_device()` ships in phase 7 (7.2). **No new code here** — this step is
