...
| Qwen3 in six sizes from 0.5 GB | [`catalog/local/manifest/models.json`](../../../surfsense_local/backend/modules/llm/catalog/local/manifest/models.json); the 0.5 GB figure came from the Ollama catalog (`qwen3:0.6b`), which the released 2.0.x builds still ship. The llama.cpp runtime on `dev` replaces it. Its manifest holds six Qwen3 sizes and Gemma 3 4B; the 0.6B's default build, `Qwen3-0.6B-UD-Q4_K_XL.gguf`, is 405,372,608 bytes (0.4 GB), and its smallest pinned build, `Qwen3-0.6B-Q2_K.gguf`, 296,238,784 bytes (0.3 GB), so the README's figure changes with the first release that carries that runtime |
