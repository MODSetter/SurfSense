...
| Qwen3 in six sizes from 0.5 GB | [`curated-models.json`](../../../surfsense_local/backend/modules/llm/catalog/local/manifest/models.json); the 0.5 GB figure came from the Ollama catalog (`qwen3:0.6b`), which the released 2.0.x builds still ship. The llama.cpp runtime on `dev` replaces it; its smallest file, `Qwen3-0.6B-Q4_K_M.gguf`, is 396,705,472 bytes (0.4 GB), so the README's figure changes with the first release that carries that runtime |
