...
- [ ] Purchase end to end in Stripe test mode: Checkout to webhook to Keygen to file on the success page **and in the inbox** to `GET /license/status` showing the plan; resend and trial flows end to end (A8 + B2 + B3). The inbox leg needs the founder's SMTP strings filled in — until then the adapter runs as `null` and resend/trial answer `503`, which is the gate, not a bug.
