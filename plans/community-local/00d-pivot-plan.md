...
| Who | Build | Verify |
|---|---|---|
| Dev A | A12 Windows installer (over the 2 GB NSIS limit); A7 `allowPrerelease = true` missing in `updater.ts` | real `v*` tag on five targets, signed and notarized; auto-update to a dummy 2.0.1 on Mac, Windows, AppImage; legacy v0.0.40 against `sunset: true` opens `/sunset`; import of a real export offline |
| Dev B | B4 sunset flag behaviour; B6 landing with a downloads section; B7 runbook; B8 launch email and reminder; B9 purge script; rest of `/sunset` (download links, import steps, refund offer) | compose stack with flags set and unset |
...
- [x] Keychain and egress merged; every outbound destination off by default (A9, A10).
- [ ] Portal live behind the flags: `/sunset`, `/pricing`, `/license/success`, `/license`, downloads, landing (B4-B6). Done: `/pricing`, `/license/success`, `/license`, `/sunset` export.
...
- [x] Studio: every MVP artifact type builds from the packaged app, not just from the dev tree (C1-C6).
