...
|---|---|---|
| **A - the app** | Dev A | `surfsense_local/`: rename, packaging, release channel split, import, license module, keychain, egress panel, auto-update, Studio review |
...
- **One repo, two update channels.** Both apps publish to `MODSetter/SurfSense`. Legacy clients (`allowPrerelease` off) ask GitHub for `/releases/latest` and read `latest.yml` from it; the new app sets `autoUpdater.allowPrerelease = true`, which in electron-updater means "take the highest semver in the releases feed" (verified in `GitHubProvider.getLatestVersion`), and publishes its manifests under its own channel (`publish.channel: stable` gives `stable.yml`, `stable-mac.yml`, `stable-linux.yml`). Legacy `v0.0.40` is pinned as the repo's latest release (`gh release edit v0.0.40 --latest`) and every later release is published with "Set as the latest release" **unchecked**. If the pin is ever lost, legacy clients hit the new app's release, find no `latest.yml`, and do nothing. Consequences: the new app must never publish a prerelease-semver tag (it would be offered to everyone), no legacy release is ever published after launch day, the GitHub "Latest" badge stays on v0.0.40, and `/downloads` links to the new app's release by tag, never to `/releases/latest`. The tag trigger moves from `local-v*` to `v*` **in the same PR that deletes** [desktop-release.yml](../../.github/workflows/desktop-release.yml) (which also fires on `v*`). Versions start at **2.0.0**: `v1.0.0` through `v1.0.98` already exist as tags in this repo.
...
- **Plugins.** The scraper client, license mode on the scraper API and on MCP all ship at T+7 as 2.1.0. The license module, purchase flow, trial, and all three frozen contracts ship at 2.0.0 so T+7 is a drop-in.
...
flowchart TD
  ...
  Legacy[Legacy desktop clients] -->|"/releases/latest, pinned to v0.0.40"| Releases
```
...
1. **License file** (`contracts/01-license-file.md`). A Keygen license file checked out with `ttl=null`. The app verifies it offline with the Keygen account public key and never contacts Keygen. Fields the app reads: policy (trial, individual, team), `expiry`, `maxUsers`, licensee email. Nothing else. Server-side metadata (`email`, `stripe_customer_id`, `checkout_session_id`) is not part of the file the app reads.
...
| `surfsense_backend/`, `surfsense_web/`, `surfsense_mcp/`, compose and infra, `plans/community-local/contracts/` | Dev B |
...
flowchart LR
  ...
  A2["A2 channel split, v0.0.40, v* switch"]
...
- **There is no wall-clock wait left.** A2 used to wait for legacy-client uptake; the channel split (pinned v0.0.40 plus the new app's own manifest names) removes that. v0.0.40 is built early and **published on launch day** (its sunset screen is itself an announcement, and there is none before launch).
- **The founder's two accounts are on the critical path** (Keygen for A8, B2, B3; Resend for B3). Day one, before either dev reaches those steps.
...
2. **Channel split and legacy updater.** In the new app: `publish.channel: stable` in `electron-builder.yml` and `autoUpdater.allowPrerelease = true` in the updater setup; version `2.0.0`. Build `surfsense_desktop` **v0.0.40** first, with the updater disabled and a sunset screen pointing at `/sunset`, by pushing the `v0.0.40` tag: [desktop-release.yml](../../.github/workflows/desktop-release.yml) signs Windows only on `v*` tag pushes, and electron-builder leaves the GitHub release as a **draft**. Then, in one PR: delete `desktop-release.yml` and switch [release-local.yml](../../.github/workflows/release-local.yml) from `local-v*` to `v*`; from here a `v*` tag builds only the new app. On launch day the v0.0.40 draft is published first and pinned as latest (`gh release edit v0.0.40 --latest`); 2.0.0 is published after it with "Set as the latest release" unchecked. Write both steps into the release checklist; every future publish repeats the unchecked box. Confirm the bundled electron-updater picks the highest semver in the feed rather than the first entry (older 6.x builds took the first entry, which would break if v0.0.40 is ever edited after a 2.x release); upgrade it if not.
3. **Offline verify.** Docling is already in `extraResources` (`build:parser`); [api/05-packaging.md](api/05-packaging.md) has the details and `HF_HOME` is redirected in [parsing.py](../../surfsense_local/backend/worker/ingestion/parsing.py). Confirm a PDF ingests on a machine with networking disabled. Fix whatever that reveals; this is verification, not packaging.
4. **First real dry run.** No `local-v*` tag has ever been cut; `release-local.yml` has never executed for real. Run it via `workflow_dispatch` on all five targets now, before any feature depends on it. Expect signing, notarization and size surprises, and check that the draft carries `stable*.yml` and no `latest*.yml`.
...
4. **Sunset flags.** Backend `SUNSET_MODE=1`: every write route (chat, uploads, connector syncs, workspace and document mutations) returns `410 Gone` with a pointer to `/sunset`; export, license and scraper routes are untouched. Web `NEXT_PUBLIC_SUNSET_MODE=1`: [ZeroProvider](../../surfsense_web/components/providers/ZeroProvider.tsx) renders children without connecting (it returns `null` for non-public routes when `zero-cache` is down, so a pass-through is required, not a removal), every app route redirects to `/sunset`, portal pages are served. Unset, both are no-ops. Portal pages use no session.
5. **Portal pages.** `/sunset` (post-login landing inside the hosted app: what changed, export button, "install SurfSense and import" steps, refund-or-discount offer, the T+30 deletion date, the T+7 MCP change), `/pricing` rewritten with buy buttons that go straight to Stripe Checkout, the early-bird end date, copy stating the first plugin arrives the week after launch, and a trial email form, `/license/success` (serves the file, explains where to put it, says a copy was emailed), `/license` (resend form and trial form, no login), `/downloads` (links to the new app's release by tag, never `/releases/latest`, which stays on the legacy v0.0.40; notes that deb does not auto-update). Unpublish `/connectors`. `/mcp-server` is rewritten for license mode at T+7.
6. **Landing rewrite** for the new positioning.
7. **Export-only switch and infra runbook.** Test both flags on the compose stack before launch, once with the flags set (sunset behaviour) and once unset (self-host behaviour unchanged). Then the runbook: stop `celery worker`, `celery beat`, `zero-cache`, `searxng`, OpenSandbox, and all LLM and embedding spend. Keep `postgres`, `redis` (verified: it backs the Google Search IP pool in [pool_store.py](../../surfsense_backend/app/proprietary/platforms/google_search/pool_store.py) and the gateway rate limiter), `backend`, `frontend`, `caddy`, and the proxy and captcha providers the scrapers need, so MCP and the scraper API run **unchanged** through the gap week. **Rollback** is the flags off and `compose up` on the stopped services; nothing is deleted until T+30.
...
- [ ] Release pipeline green on all five targets from a real `v*` tag, signed and notarized, draft carrying `stable*.yml` only (A2, A4).
- [ ] `desktop-release.yml` deleted, `v*` trigger live, legacy v0.0.40 built and waiting for launch day (A2).
...
- [ ] Auto-update: 2.0.0 to a dummy 2.0.1 succeeds on Mac (zip), Windows and AppImage with a legacy release pinned as latest; a legacy 0.0.39 client pointed at the same repo does nothing (A7).
...
- **Legacy desktop clients pull the new app.** Two independent guards: the pinned v0.0.40 keeps `/releases/latest` on the legacy app, and the new app ships no `latest.yml`, so a lost pin means a 404 and nothing happens. Residual: users who never open the legacy app keep a client pointing at a dead backend until they do, at which point v0.0.40's sunset screen greets them. Human step to protect: the "Set as the latest release" box stays unchecked on every 2.x publish.
