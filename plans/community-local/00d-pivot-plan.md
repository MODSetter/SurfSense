...
Every item below is a decision, not an assumption. Work is ordered, not dated; launch is the day the launch gates are green. The investor-facing target window (Sept 14-18, 2026) lives in `plans/yc-application/`; this document does not repeat it, and the gates decide. This document is the single source for the pivot. The phase docs in this folder ([`00-umbrella-plan.md`](00-umbrella-plan.md), `api/`, `worker/`, `frontend/`) remain the technical specs for the local app itself, and `portal/` holds Workstream B's backend specs; "community-local" stays as the internal folder name, the user-facing name is **SurfSense**.
...
| Who | Build | Verify |
|---|---|---|
| Dev A | A12 Windows installer (over the 2 GB NSIS limit) — the only target still failing; A7 `allowPrerelease = true` missing in `updater.ts`; A8 compiles in the `license-sample` **test** public key and its guard test never runs (`addopts` sets `-m 'not packaging'` and CI runs only `-m unit` and `-m integration`); A10 `POST /llm/install` reaches `registry.ollama.ai` without `egress.require()`; A5 has no import summary endpoint | real `v*` tag on all four targets, signed and notarized — A4 dry run so far: Mac and Linux green, Windows fails at `Package installer`; auto-update to a dummy 2.0.1 on Mac, Windows, AppImage; legacy v0.0.40 against `sunset: true` opens `/sunset`; import of a real export offline |
...
- Marketing: rewrite **landing, pricing, downloads**; unpublish `/connectors`; rewrite `/mcp-server` for license mode at T+7; keep the blog. **Still open:** the landing's actual content (deferred to the founder's B6 brief, not yet written) and the disposition of eight other public routes — `/free` above all, a no-login hosted AI chat funnel that dropping hosted inference kills. Logged in [`portal/02-pages.md`](portal/02-pages.md), "Open items".
...
2. **Legacy v0.0.40 and the channel split.** `surfsense_desktop` v0.0.39 auto-downloads whatever GitHub calls the latest release, tag name irrelevant, so it must be neutralised before the new app publishes anything. v0.0.40 is deliberately tiny and has **no sunset content of its own**: the legacy app bundles a frozen copy of `surfsense_web` (see `extraResources` in [surfsense_desktop/electron-builder.yml](../../surfsense_desktop/electron-builder.yml)), so any page baked into it would be stale the day Dev B changes the portal. Instead:
   ...
   Then, in one PR: delete `desktop-release.yml` and switch [release-local.yml](../../.github/workflows/release-local.yml) from `local-v*` to `v*`; from here a `v*` tag builds only the new app. In the new app: `publish.channel: stable` in `electron-builder.yml`, so it reads `stable-*.yml` and the legacy app's `latest-*.yml` feed is never written again; the version comes from the tag, not `package.json`. Every 2.x release is published with "Set as the latest release" unchecked; write that into the release checklist. Done: v0.0.40 is live and pinned, `desktop-release.yml` is deleted, `v*` builds the new app, `docker-build.yml` only fires on `v0.*`.
...
4. **First real dry run.** No `local-v*` tag has ever been cut; `release-local.yml` has never executed for real. Run it via `workflow_dispatch` on all four targets now, before any feature depends on it, with `publish: never` or by leaving the draft unpublished; either way nothing reaches a client. Expect signing, notarization and size surprises, and check that the artifacts carry `stable*.yml` and no `latest*.yml`. **Done in part:** `workflow_dispatch` runs got Mac and Linux green; Windows fails at `Package installer`, which is A12. The `stable*.yml` check is still open because no `v*` tag has been cut.
...
8. **License module.** New `modules/license/`: import file, verify offline with the embedded public key, persist, clock-rollback watermark (highest timestamp seen in SQLite; earlier clock marks the license untrusted), `GET /license/status`. Settings UI: drop or paste the file, plan and expiry, and two links that open the portal in the browser: "Start 14-day trial" and "Buy". The app has no trial or purchase logic of its own; the portal emails the file. Needs a real Keygen-signed test file, so Dev B's Keygen CE instance precedes it (a local instance is enough; no signup). **Open, and a release blocker:** `verify.py` still compiles in the Ed25519 key from `contracts/license-sample/public-key.hex`, so anyone with this repo can sign a license the app accepts. `tests/packaging/test_license_key.py` exists to catch exactly that, but `pyproject.toml` sets `addopts = -m 'not packaging'` and CI runs only `-m unit` and `-m integration`, so the guard never fires. Swapping in the account key and giving the `packaging` marker a CI job are one task.
...
**The rule was not held for four formats.** All eleven MVP artifact types build end to end with integration tests, but DOCX, PPTX, XLSX and PDF went the other way: `worker/studio/office/prompt.py` asks the model for "one standalone Python script" and `worker/studio/office/runner.py` runs it with `exec()` in the worker process, unsandboxed, bounded only by a 120s timeout that cannot hard-kill a runaway thread. The module says so plainly and carries a `ponytail:` comment naming the ceiling, so it was deliberate — it is still the pattern this rule existed to keep out, and it puts the accepted risk back at "arbitrary code with the worker's privileges." Summary, mind map, flashcards, quiz, HTML, podcast and infographic follow the rule. Three smaller deltas: PDF uses ReportLab through that same `exec()` path rather than Electron `printToPDF` (which appears nowhere in the repo, so C4 as specified is unbuilt); XLSX uses `xlsxwriter`, not `openpyxl`; and the frontend renders mind map, flashcards, quiz and HTML as plain markdown, with no Markmap, no interactive cards, and no sandboxed iframe for the HTML artifact.
...
6. **Infographics.** BYO image-model key stored through the keychain path; enabled only when its egress toggle is on. Depends on A9 and A10, which is why it is last. **Shipped differently:** `infographic` asks the generation model for structured JSON and renders it as deterministic SVG, so it needs no image key and no egress; the separate `image` format is the one that resolves a BYO key through the keychain and calls `egress.require()`. Either decide `infographic` stays offline and drop the A9/A10 dependency from this item, or wire it to the image path as written.
...
- [ ] Release pipeline green on all four targets from a real `v*` tag, signed and notarized, artifacts carrying `stable*.yml` only (A2, A4). Dry run stands at Mac and Linux green, Windows failing at `Package installer` (A12); no `v*` tag cut yet, so the `stable*.yml` leg is unverified.
...
- [ ] Keychain and egress merged; every outbound destination off by default (A9, A10). Merged, but `POST /llm/install` still pulls through `runtime.install(plan)` without `egress.require()`, so the "every destination" half is not yet true.
- [ ] Portal live behind the flags: `/sunset`, `/pricing`, `/license/success`, `/license`, downloads, landing (B4-B6). Done: `/license/success`, `/license`, `/sunset` export. **Not done, despite an earlier "done" note here:** `/pricing` still sells pay-as-you-go credits with no license prices, early-bird date or trial form, and there is no downloads route at all.
...
- **T-0, the day the launch gates are green:** publish SurfSense 2.0.0 on all four targets with "Set as the latest release" unchecked (the legacy release keeps the pin); **retitle and rewrite the pinned `v0.0.40` release** so the releases page stops handing browsers the dead hosted app — it keeps GitHub's `Latest` badge forever by design, so its title must say it is the discontinued legacy client and its body must link 2.0.0 by tag and `/downloads` (`gh release edit v0.0.40 --title ... --notes-file ...`). **Not before T-0:** that page is public, so editing it earlier announces the pivot ahead of the launch email. Then set both sunset flags on the hosted deployment, which also turns every legacy v0.0.40 into the live `/sunset` page on next launch; run the scale-down runbook; auto-reload off; purchase and trial open; MCP and scraper API keep serving unchanged. The launch email and the in-app banner go out the same day - the only announcement: what changed, download link, export and import steps, refund-or-discount offer, the T+30 deletion date, and the T+7 MCP change. Users export, install, import - the migration is complete on day one.
